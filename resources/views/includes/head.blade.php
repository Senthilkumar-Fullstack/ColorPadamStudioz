<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width" />
<meta name="viewport" content="initial-scale=1.0" />
<title>1ClickStudios | Build Memories</title>
<link rel="icon" type="image/png" href="{{ URL::asset('resources/assets/img/favicon.png') }}">
<link rel="stylesheet" type="text/css" href="{{ URL::asset('resources/assets/lib/bootstrap/css/bootstrap.min.css') }}">
<link rel="stylesheet" type="text/css" href="{{ URL::asset('resources/assets/lib/bootstrap/css/bootstrap-theme.min.css') }}">
<link rel="stylesheet" type="text/css" href="{{ URL::asset('resources/assets/dist/css/styles.css') }}">
<link rel="stylesheet" type="text/css" href="{{ URL::asset('resources/assets/lib/OwlCarousel2/dist/assets/owl.carousel.min.css') }}">
<link rel="stylesheet" type="text/css" href="{{ URL::asset('resources/assets/lib/OwlCarousel2/dist/assets/owl.theme.default.min.css') }}">